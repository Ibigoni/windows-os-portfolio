...
export default function Window({ win, children }: { win: WindowState; children: React.ReactNode }) {
  ...
  // minimized windows are "running" but not visible
  if (win.isMinimized) return null;

  const baseStyle: React.CSSProperties = win.isMaximized 
  ? { top: 0, left: 0, width: "100w", height: "calc(100vh - 48px" }
  ...
  return (
    <Draggable
      ...
      onStop={(_: DraggableEvent, data: DraggableData) => moveWindow(win.id, data.x, data.y)}
    >
      {content}
    </Draggable>
  );
